...
export default function WordClient() {
  ...
  const handleConvert = async () => {
    ...
    setResult(null);
    try {
      const blob = await convertToWord(file);
      setResult(blob);
    } catch (e) {
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 py-12 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm pl-[42px]" style={{ color: "#6B7355" }}>
            Extract text from each page and export as a .docx file — processed on the server.
          </p>
        </div>
        ...
        {result && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <p className="font-semibold text-gray-900 text-sm">Converted successfully</p>
            </div>

            <button
              onClick={() => triggerBlobDownload(result, `${baseName}.docx`)}
